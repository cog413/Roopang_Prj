...
const PLAYED_KEY = 'sdk_sudoku_played';
...
export async function initSudoku() {
    ...
    function buildScorePanel() {
        const rightPanel = document.querySelector('#sudoku-sheet .side-right');
        ...
        if (progressTable) {
            ...
            scoreValue.id = 'sudoku-score-display';
        ...
        chart.innerHTML = `
            ...
            <div class="chart-bars">
                ...
                <div class="bar-container"><div class="bar" id="sudoku-score-bar" style="height:5%"></div><span>Total</span></div>
            </div>`;
    ...
    async function loadPuzzle(difficulty) {
        ...
        if (formulaInput) formulaInput.value = `=SUDOKU.LOAD("${row.puzzle_id}",D=${row.difficulty})`;
    ...
    function resetScoreUI() {
        ...
        // Reset sub bars
        const bars = document.querySelectorAll('#sudoku-sheet .fake-chart .bar:not(#sudoku-score-bar)');
